...
export async function resolveTownDisplayName(townId: string | null | undefined): Promise<string | null> {
  if (!townId) return null;
  const towns = await listEnabledTowns();
  return towns.find((t) => t.id === townId)?.displayName ?? null;
}
