...
public class CreateHubRequest {
    ...
    @AssertTrue(message = "Reference phones must differ from admin phone and each other")
    public boolean isReferencePhonesDistinct() {
        if (phone == null || reference1Phone == null || reference2Phone == null) {
            return true;
        }
        String admin = (adminPhone == null || adminPhone.isBlank()) ? phone.trim() : adminPhone.trim();
        String r1 = reference1Phone.trim();
        String r2 = reference2Phone.trim();
        return !r1.equals(admin) && !r2.equals(admin) && !r1.equals(r2);
    }
}
